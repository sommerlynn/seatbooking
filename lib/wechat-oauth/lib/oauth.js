...
OAuth.prototype._getJsAPITicket = function(accessToken, callback){
...
OAuth.prototype.getJsApiTicket = function(options, callback){
  if (typeof options !== 'object') {
    options = {
      openid: options
    };
  ...
  this.getToken(options.openid, function (err, data) {
    ...
    var token = AccessToken(data);
    if (token.isValid()) {
      that._getJsAPITicket(options, token.data.access_token, callback);
    } else {
      that.refreshAccessToken(token.data.refresh_token, function (err, token) {
        if (err) {
          return callback(err);
        }
        that._getJsAPITicket(options, token.access_token, callback);
      });
    }
  });
};
