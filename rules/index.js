        ...
        handler: function(info){
            var reply = {
                ...
                description: [
                    ...
                    '维度:'+info.param.lat,
                    '经度:'+info.param.lng,
                    '时间:'+new Date().toLocaleString('en-GB')
                ].join('\n')
            };

            log('维度', info.param.lat);
            log('经度', info.param.lng);
            log('时间', new Date().toLocaleString('en-GB'));

            info.noReply = true;
            // 返回值如果是list，则回复图文消息列表
            models.weixinMessageModel.logUserLocation(info.uid, info.param.lat, info.param.lng, function(err){
                if(err){
