...
router.post('/libraryClassroom/seat/log', function(req, res) {
    models.seatModel.getLogByDateType(req.body.classroomID, req.body.seatCode, req.body.type, function (err, seatLogs) {
        for(var index = 0; index < seatLogs.length; index++){
            seatLogs[index].log_time = seatLogs[index].log_time.toLocaleString('en-US', {hour12:false});//(seatLogs[index].log_time.getMonth()+1)+'/'+seatLogs[index].log_time.getDate()+ ' ' + seatLogs[index].log_time.getHours() + ':' +seatLogs[index].log_time.getMinutes();
        }
